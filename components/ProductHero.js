...
export default function ProductHero({ product }) {
  ...
  }, [product, quantity, selectedVariant, selectedBundle]);
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-4 py-8 sm:py-4 lg:py-9 sm:px-6 lg:px-8 font-anek">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-5">
        {/* Product Image */}
        <div
          className="bg-[#eeeeee] border border-[#e8e8e8] rounded-[25px] lg:rounded-[24px] p-0 flex items-center justify-center overflow-hidden transition-all duration-300 min-h-[400px] sm:min-h-[300px]"
          style={{ minHeight: rightHeight && window.innerWidth >= 640 ? rightHeight : undefined }}
        >
          <Image
        ...
        >
          ...
          <div className="lg:flex lg:items-center lg:gap-6 mb-6">
            <div className="flex-1">
              ...
              <div className="flex items-center mb-3">
                <span className="text-sm font-semibold text-black w-20">Dosage</span>
              ...
              <div className="flex items-center">
                <span className="text-sm font-semibold text-black w-20">Quantity</span>
                <div className="flex items-center border border-[#e0e0e0] rounded-full">
                  <button onClick={decreaseQty} className="w-9 h-9">−</button>
                  <span className="w-8 text-center text-sm font-medium">{quantity}</span>
                  <button onClick={increaseQty} className="w-9 h-9">+</button>
                </div>
              </div>
            </div>

            <div className="text-right">
              <span className="text-2xl font-bold text-black">${finalPrice}</span>
            </div>
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center gap-3 px-6 py-4 border rounded-lg transition-all flex-shrink-0 ${
                        isSelected ? "border-black bg-gray-100" : "border-[#e0e0e0] hover:border-black"
                      }`}
                    >
                      {bundle.tag && (
                        <span
                          className={`absolute -top-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full text-[10px] font-semibold text-white whitespace-nowrap z-10 ${
                            bundle.tag.toLowerCase() === "most popular" ? "bg-teal-600" : "bg-amber-500"
                          }`}
                        >
                          {bundle.tag}
                        </span>
                      ...
                      <div className="flex items-center h-12">
                        {[...Array(bundle.bottles)].map((_, i) => (
                          <Image
                            ...
                            width={22}
                            height={44}
                            className={`object-contain ${bundle.bottles > 1 ? "-mr-1" : ""}`}
                          />
                        ))}
                      </div>
          ...
          <div className="flex justify-between items-start gap-2 mt-3 py-3 px-3 bg-[#f8f8f8] rounded-xl text-[#555]">
            ...
            <div className="flex flex-col items-center text-center gap-1 flex-1">
              ...
              <span className="text-[11px] leading-tight">Tue, Mar 3 – Mon, Mar 9</span>
            </div>
            ...
            <div className="flex flex-col items-center text-center gap-1 flex-1">
              ...
              <button type="button" className="text-[11px] leading-tight inline-flex items-center gap-0.5 cursor-help">
                <span className="underline decoration-dotted decoration-gray-400 underline-offset-2">Free shipment protection</span>
                <span className="w-3 h-3 flex items-center justify-center rounded-full bg-gray-300 text-[7px] font-bold text-gray-600">?</span>
              </button>
            </div>
            ...
            <div className="flex flex-col items-center text-center gap-1 flex-1">
              ...
              <span className="text-[11px] leading-tight">Overnight shipping available</span>
            </div>
          </div>

          {/* Klarna */}
          <div className="flex items-center gap-2 bg-[rgba(255,179,199,0.1)] border border-[#FFB3C7]/30 rounded-lg px-3 py-2.5 mt-4">
            <img src="/images/product-list/pink.svg" alt="Klarna" width={55} height={24} className="flex-shrink-0"/>
            <span className="text-sm text-[#333] leading-tight">
              4 interest-free payments of <span className="font-semibold">${(finalPrice / 4).toFixed(2)}</span>
            </span>
          </div>
...
function Star() {
  return (
    <svg className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
